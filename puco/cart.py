class Cart:
    def __init__(self,request):
        ...
        cart1=self.session.get("cart1")
        if not cart:
            cart=self.session["cart"]={}
        self.cart=cart
        if not cart1:
            cart1=self.session["cart1"]={}
        self.cart1=cart1
    ...
    def add1(self,ofertas):
        if str(ofertas.get("id")) not in self.cart1.keys():
            self.cart1[ofertas.get("id")]={
            ...
            for key,value in self.cart1.items():
    ...
    def remove1(self,ofertas):
        ...
        if ofertas_id in self.cart1:
            del self.cart1[ofertas_id]
        self.save()
        self.calcular()
